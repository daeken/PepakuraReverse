...
def main(fn):
    ...
    scale = 5
    part = 0
    with open(f"{directory}/linedraw.html", "w") as f:
        with redirect_stdout(f):
            while True:
                ...
                offsety = abs(miny) + 2
                print(
                    '<canvas id="cvs%i" width=%i height=%i></canvas><br><br><script>'
                    % (
                        part,
                        (abs(maxx) + abs(minx)) * scale + 4 * scale,
                        (abs(maxy) + abs(miny)) * scale + 4 * scale,
                    )
                )
                ...
                for obj in data["geometry"]:
                    for elem in obj["shapes"]:
                        ...
                        print(
                            "ctx.moveTo(%f, %f);"
                            % (
                                (points[-1][0] + offsetx) * scale,
                                (points[-1][1] + offsety) * scale,
                            )
                        )
                        for point in points:
                            print(
                                "ctx.lineTo(%f, %f);"
                                % (
                                    (point[0] + offsetx) * scale,
                                    (point[1] + offsety) * scale,
                                )
                            )
                        print("ctx.stroke();")
                print("</script>")
